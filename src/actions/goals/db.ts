...
export const dbCreateGoal = async (userId: string, title: string, deadline: Date) => {
    const res = await db.insert(goalsTable).values({
        user_id: userId,
        title,
        completed: false,
        deadline
...
export const dbMarkGoalAsComplete = async (userId: string, goalId: string, completed: boolean) => {
    const res = await db.update(goalsTable).set({
        completed: completed
    }).where(
        and(
            eq(goalsTable.id, goalId),
